...
<body>
    ...
    <section>
        <form id="formid" method="post" action="../../php/authentication.php">
            <input name="username" type="text" maxlength="128" placeholder="Username">

            <input id="password" name="password" type="password" maxlength="128" placeholder="Password">

            <input id="signup1" type="submit" name="submit" value="Sign up" onclick="return validate()">

            <?php 
                $p_error = 'id="p-error"';
                
                if(isset($_GET['success'])){
                    if($_GET['success']== "wrongpwd")
                        echo "<p $p_error>username already in use</p>";
                    else if($_GET['success']== "duplicateusername"){
                        if(isset($_GET['username'])){
                            $name = $_GET['username'];
                            echo "<p $p_error>Hi $name, do you want to <a href=login.php>log in</a>?</p>";
                        }   
                    }            
                }  
            ?>
